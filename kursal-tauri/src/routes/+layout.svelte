<script lang="ts">
  ...
  function refreshTitle() {
    document.title = backgroundUnread > 0
      ? `(${backgroundUnread}) ${baseTitle}`
      : baseTitle;
  }
  ...
  onMount(() => {
    ...
    unlistenPromises.push(listen<MessageReceivedPayload>('message_received', (event) => {
      messagesState.append(event.payload);
      if (document.hidden) {
        backgroundUnread += 1;
        refreshTitle();
      }
    ...
    unlistenPromises.push(listen<ConnectionChangedPayload>('connection_changed', (event) => {
      contactsState.setConnectionStatus(event.payload.contactId, event.payload.status);
    }));

    // Listen to contact_added event
    unlistenPromises.push(listen<ContactResponse>('contact_added', (event) => {
    ...
    unlistenPromises.push(listen<{ messageId: string; contactId: string }>('delivery_confirmed', (event) => {
      messagesState.updateStatus(event.payload.messageId, event.payload.contactId, 'delivered');
    }));

    // Listen to message_edited event
    unlistenPromises.push(listen<MessageEditedPayload>('message_edited', (event) => {
      messagesState.updateContent(event.payload.messageId, event.payload.contactId, event.payload.newContent);
    }));

    // Listen to message_deleted event
    unlistenPromises.push(listen<MessageDeletedPayload>('message_deleted', (event) => {
      messagesState.markDeleted(event.payload.messageId, event.payload.contactId);
    }));

    // Listen to reaction_added event
    unlistenPromises.push(listen<ReactionChangedPayload>('reaction_added', (event) => {
      messagesState.addReaction(event.payload.messageId, event.payload.contactId, event.payload.emoji);
    }));

    // Listen to reaction_removed event
    unlistenPromises.push(listen<ReactionChangedPayload>('reaction_removed', (event) => {
      messagesState.removeReaction(event.payload.messageId, event.payload.contactId, event.payload.emoji);
    }));

    // Listen to peer_id_rotated event (OUR OWN rotation was published)
    unlistenPromises.push(listen<string[]>('peer_id_rotated', (event) => {
    ...
    unlistenPromises.push(listen<ContactResponse>('contact_updated', (event) => {
      ...
      if (oldPeerId && oldPeerId !== newContact.peerId) {
        notifications.push(`${newContact.displayName} rotated their peer ID.`, 'info');
      }
      ...
      if (nameChanged && avatarChanged) {
        notifications.push(`${oldName} changed their name to ${newContact.displayName} and updated their profile picture`, 'info');
      } else if (nameChanged) {
        notifications.push(`${oldName} changed their name to ${newContact.displayName}`, 'info');
      } else if (avatarChanged) {
        notifications.push(`${newContact.displayName} updated their profile picture`, 'info');
      }
    }));

    // Listen to ltc_expiring_soon event
    unlistenPromises.push(listen<number>('ltc_expiring_soon', (event) => {
      const hours = event.payload;
      notifications.push(`Contact file expires in ${hours} hour${hours === 1 ? '' : 's'}.`, 'info');
    }));

    // Listen to nearby_request event
    unlistenPromises.push(listen<NearbyRequestPayload>('nearby_request', (event) => {
